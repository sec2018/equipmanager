 ...
 * @version 2018-08-23
 */
@Table(name="maintain_record_child", alias="a", columns={
		@Column(name="id", attrName="id", label="编号", isPK=true),
		@Column(name="record_sort", attrName="recordSort", label="排序号"),
		@Column(name="record_code", attrName="recordCode.recordCode", label="工单编号"),
...
public class MaintainRecordChild extends DataEntity<MaintainRecordChild> {
	
	private static final long serialVersionUID = 1L;
	private Long recordSort;		// 排序号
	private MaintainRecord recordCode;		// 工单编号 父类
	...
	public MaintainRecordChild() {
		this(null);
	}


	public MaintainRecordChild(MaintainRecord recordCode){
		this.recordCode = recordCode;
	}
	
	public Long getRecordSort() {
		return recordSort;
	}

	public void setRecordSort(Long recordSort) {
		this.recordSort = recordSort;
	}
	
	@Length(min=0, max=64, message="工单编号长度不能超过 64 个字符")
	public MaintainRecord getRecordCode() {
		return recordCode;
	}

	public void setRecordCode(MaintainRecord recordCode) {
		this.recordCode = recordCode;
	}
